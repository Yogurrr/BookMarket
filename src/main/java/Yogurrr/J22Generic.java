...
public class J22Generic {
    ...
    public static void main(String[] args) {
        ...
        // lionzoo.add(new Tiger());   // 컴파일타임 오류 발생!

        // 자동 형변환이 되므로 오류 없이 실행됨
        Lion l2 = lionzoo.get(0);

        // 제네릭 타입
        // 임의의 숫자를 입력 받아 더하는 메서드를 만듦
        ...
        // 실행 중에 필요에 따라 타입을 매개변수 형태로 받아 사용함
        // 비슷한 기능을 지원하는 코드들을 간략하게 작성할 수 있어
        // 코드의 재사용성이 높아짐
        System.out.println(example.add(new Integer(40), new Integer(54)));
        System.out.println(example.add(new Double(4.3), new Double(5.4)));
        System.out.println(example.add(new Float(9.8), new Float(7.6)));
    }
}
...
class Adds {
    ...
    public <T> T add(T a, T b) {
        T sum = null;
        if (a.getClass() == Integer.class) {   // a 변수의 클래스형이 Integer라면
